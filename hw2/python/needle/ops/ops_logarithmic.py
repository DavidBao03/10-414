...
class LogSumExp(TensorOp):
    ...
    def compute(self, Z):
        ### BEGIN YOUR SOLUTION
        z_origin = array_api.max(Z, axis=self.axes, keepdims=True)
        z_reduce = array_api.max(Z, axis=self.axes)
        Z = Z - z_origin
        return array_api.log(array_api.sum(array_api.exp(Z), axis=self.axes)) + z_reduce
        ### END YOUR SOLUTION
